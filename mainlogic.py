    ...
    def select_photo(self): #Фото
        try:
            ...
            if filename:
                ...
                w, h = im.size
                if w <= 500 and h <= 500:  # можно настроить размер
                    self.ui.lineEdit_8.setText(filename)
                else:
                    QMessageBox.critical(self, 'Ошибка', 'Размер изображения превышает 500x500 пикселей.', QMessageBox.Ok)
    ...
    def read_partners(self):
        ...
        for row in range(len(self.books_data)):
            ...
            if total_orders < 10000:
                ...
                item.setText('15%')
            self.ui.tableWidget.setHorizontalHeaderLabels(['Информация о книге', 'Скидка', 'Фото'])
